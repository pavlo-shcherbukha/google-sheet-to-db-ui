<template>
	<div class="card material-table" :style="{width: tableWidth ? tableWidth : 'auto'}">
		...
		<div class="table-footer">
			<p>строка={{ curentRow }} из {{tableRows.length}}. Ключеое поле {{KeyField}} Это footer грида </p>
...
<script>
	export default {
		...
		watch: {
			curentRow: function( newval, oldval){
				console.log('oldval=' + oldval +' ==> new val=' + newval);
				this.setRow( newval );
                          
			},
		},
		updated(){
			console.log('вызываю событие dtb_updated');
			this.$emit('dtbupdated');
		},
		methods: {
			/**
